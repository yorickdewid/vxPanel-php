...
# Lets check for some common control panels that we know will affect the installation/operating of QPanel.
if [ -e /usr/local/cpanel ] || [ -e /usr/local/directadmin ] || [ -e /usr/local/solusvm/www ] || [ -e /usr/local/home/admispconfig ] || [ -e /usr/local/lxlabs/kloxo ] ; then
    ...
    echo "is designed to install and configure QPanel on a clean OS installation only!"
    echo ""
...
echo -e "# any packages required by QPanel.                           #"
echo -e "#                                                            #"
...
while true; do
	...
	tz=`cat /etc/timezone`
	echo -e "Enter the FQDN you will use to access QPanel on your server."
...
# Set-up QPanel directories and configure directory permissions as required.
mkdir /etc/zpanel
...
echo -e "##############################################################" &>/dev/tty
echo -e "# Congratulations Qpanel has now been installed on your      #" &>/dev/tty
